...
class DataPrepper:
    ...
    def __log_ltr_query_features(self, query_id, key, query_doc_ids, click_prior_query, no_results, terms_field="_id"):
        ...
        if response and len(response['hits']) > 0 and len(response['hits']['hits']) == 1:
            hits = response['hits']['hits']
            for doc_id in hits:
                feature_results["doc_id"].append(doc_id["_id"])  # capture the doc id so we can join later
                feature_results["query_id"].append(query_id)
                feature_results["sku"].append(doc_id["_source"]["sku"][0])
                for i in doc_id["fields"]["_ltrlog"][0]["log_entry"]:
                    if i["name"] not in feature_results:
                        feature_results[i["name"]] = []
                    if "value" not in i:
                        feature_results[i["name"]].append(0)
                    else:
                        feature_results[i["name"]].append(i["value"])
